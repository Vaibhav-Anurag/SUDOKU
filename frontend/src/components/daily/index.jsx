...
import NumberControl from '../puzzle/NumberControl'
...
function getClickHandler(onClick, onDoubleClick, delay = 250) {
  ...
  return (event) => {
    if (!timeoutID) {
      ...
      const DPuzzle = () => {
        ...
        const alreadySolved = async(userId) => {
          var response;
          try{
            response = await axios.get(`${baseUrl}/leaderboard/${userId}`,{withCredentials:true});
          }catch(error){
            console.log(error);
          }
          return response;
        ...
        useEffect(() => {
          const checkIfSolved = async () => {
            ...
            try {
              const resp = await alreadySolved(userId);
              // console.log(userId);
              // console.log('resp: ', resp);
              // console.log('respexists: ', resp.data.exists);
              if (resp.data.exists != null) {
                setIsSolved(true);
              }
            } catch (error) {
        ...
        useEffect(() => {
          axios.get(`${baseUrl}`,{withCredentials:true})
            .then((response) => {
            setDailypuzzle(response.data[0]);
      ...
      const generateGame = useCallback(() => {
        if (!dailypuzzle || Object.keys(dailypuzzle).length === 0) return;
       ...
       useEffect(() => {
         if (dailypuzzle) {
           console.log('dailyPuzzle: ', dailypuzzle.puzzle, 'solution :', dailypuzzle.solution);
           generateGame();
         }
       }, [dailypuzzle, generateGame]);
        ...
        const addUserToDailyUser = async (userId) => {
          try {
            const response = await axios.post(`http://localhost:3001/leaderboard/${userId}`, {}, {
  ...
  }, [state, undo, redo, eraseSelected, getSelectedCell]);
  ...
  return (
    <div className="pbody">
      <div className="heding"><h1>Daily Puzzle</h1></div>
      ...
      <style>{`
        .pbody {
          ...
          @media(max-width:768px){
            :global(.header, .puzzle, .controls,.heding) {
              font-size: .8em;
            }
          }
        :global(pbody) {
